...
struct Boxnode : Box<DIM> {
  ...
  Boxnode(Point<DIM> mylo, Point<DIM> myhi, Int mymom, Int myd1,
          Int myd2, Int myptlo, Int mypthi) : Box<DIM>(mylo, myhi), mom(mymom), dau1(myd1), dau2(myd2),
...
template <Int DIM>
KDtree<DIM>::KDtree(vector<Point<DIM>> &pts) : ptss(pts), npts(pts.size()), ptindx(npts), rptindx(npts) {
